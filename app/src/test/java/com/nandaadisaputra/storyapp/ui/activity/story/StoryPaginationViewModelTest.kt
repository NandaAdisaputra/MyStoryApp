...
class StoryPaginationViewModelTest {
    ...
    fun `when Get Story Should Not Null`() = runTest {
        ...
        val actualStory = storyViewModel.getStory(dummyToken).getOrAwaitValue()
        ...
        advanceUntilIdle()
        assertNotNull(actualStorySnapshot)
        assertEquals(dummyStory.size, actualStorySnapshot.size)
        assertEquals(dummyStory[0].id, actualStorySnapshot[0]?.id)
    }
